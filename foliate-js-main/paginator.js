...
class View {
    ...
    constructor({ container, onExpand }) {
        ...
        // In Electron/Chromium, allow-same-origin is sufficient and allow-scripts is removed for security
...
export class Paginator extends HTMLElement {
    ...
    #afterScroll(reason) {
        ...
        if (this.scrolled) detail.fraction = this.start / this.viewSize
            ...
            detail.page = Math.max(1, page - 1)
            detail.totalPages = Math.max(1, pages - 2)
        }
    ...
    async #turnPage(dir, distance) {
        ...
        try {
            ...
            if (shouldGo) {
                const adjIndex = this.#adjacentIndex(dir)
                if (adjIndex != null && this.#canGoToIndex(adjIndex)) {
                    await this.#goTo({
                        index: adjIndex,
                        anchor: prev ? () => 1 : () => 0,
                    })
